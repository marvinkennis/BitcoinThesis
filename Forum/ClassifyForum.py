...
dataset = 'TrainingDataForumBT.csv'
...
for i in range(0,rc):
	...
	try: 
		...
		stt = df1.iloc[i]['date'].split(", ")
		newst = stt[0]+" "+stt[1]
		stt = datetime.datetime.strptime(newst, '%B %d %Y')
		ts = str(stt).split(" ")[0]
		print i
		print ts
		...
		key = ts
		publisher = df1.iloc[i]['subforum']
		publisher_total = publisher+'_total'
...
for d in list(marketresults.keys()):
	try:
		...
		with open('removeFINAL_FORUM_TOTAL.csv', 'a') as testfile:
